...
async function getPokemonsRequest ({ req: { query: { pageSize, page, types } }, res }) {
  try {
...
async function getPokemonsByNameRequest ({ req: { params: { name } }, res }) {
  try {
...
async function getPokemonByIdRequest ({ req: { params: { id } }, res }) {
  try {
    id = parseInt(id, 10) || 1
...
async function addPokemonToFavoriteRequest ({ req, res }) {
  try {
...
async function removePokemonFromFavoriteRequest ({ req, res }) {
  try {
    if (!req.session.user) return res.status(401).send('User is not authorized')
    if (!req.body.pokemonId) return res.status(400).send('Pokemon id is not specified')
    const userId = req.session.user.id
